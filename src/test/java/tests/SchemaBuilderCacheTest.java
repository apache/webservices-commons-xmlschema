...
public class SchemaBuilderCacheTest extends Assert {
    ...
    /**
     * Test if the cache is initialized it will be populated when a schema is read and it will
    ...
    public void testResolveCacheInitialized() throws Exception {
        try {
            ...
            // After clearing the cache, there should be no entry for this thread ID, and 
    ...
    /**
     * Override the protection on the resolvedSchemas attribute in the SchemaBuilder class
     * and return its current value.
     * @return value of Hashtable resolvedSchemas, which may be null
     * @throws Exception If there are problems with Java reflection (should not happen)
     */
    static Map<String, Map<String, SoftReference<XmlSchema>>> getResolvedSchemasHashtable(){
    	Map<String, Map<String, SoftReference<XmlSchema>>> ht = null;
        try {
            Field field = SchemaBuilder.class.getDeclaredField("resolvedSchemas");
            field.setAccessible(true);
            ht = (Map<String, Map<String, SoftReference<XmlSchema>>>)field.get(null);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        return ht;
    ...
    static Map<String, SoftReference<XmlSchema>> getThreadResolvedSchemaHashtable() {
        ...
        Map<String, Map<String, SoftReference<XmlSchema>>> ht = getResolvedSchemasHashtable();
        if (ht != null) {
            String threadID = String.valueOf(Thread.currentThread().getId());
            threadHashtable = (Map<String, SoftReference<XmlSchema>>) ht.get(threadID);
        }
        return threadHashtable;
    }
    
    /**
     * Set the resolvedSchemas collection to null.  This should be done in a finally block
     * of any tests that cause an SchemaBuilder.initCache to be done to cleanup for the next 
     * test.
     */
    static void resetResolvedSchemasHashtable() {
        try {
            Field field = SchemaBuilder.class.getDeclaredField("resolvedSchemas");
            field.setAccessible(true);
            Map<String, Map<String, SoftReference<XmlSchema>>> ht = (Map<String, Map<String, SoftReference<XmlSchema>>>) field.get(null);
            if (ht != null) {
                ht.clear();
                field.set(null, null);
            } 
    ...
    /**
     * Configure and start the test threads for the multi-threaded testing.  The threads will
     * perform various tests between themselves such as clearing cache in one thread and making 
     * sure the cache used by a different thread is not affected.  A monitor is used to control
     * the synchonization between the threads and for communicating faliures back to the test
     * method.  
     * 
...
/**
 * Monitor used to control synchronization between the testing threads and communicate failures
 ...
 * - Verify that the clearCache done by Thread 2 did not affect this Thread's cache.
 */
@Ignore
class TestingRunnable1 implements Runnable {
    ...
    public void run() {
        ...
        XmlSchemaCollection schemaCol = SchemaBuilderCacheTest.setupXmlSchemaCollection();
        XmlSchema schema = schemaCol.read(doc, null);
        if (schema == null) {
            testMonitor.t1Exception = new Exception("Schema was null");
        }

        // If the cache is in use, it should not be null and there should be an entry for this
        // Thread
        if (SchemaBuilderCacheTest.getResolvedSchemasHashtable() == null) {
            testMonitor.t1Exception = new Exception("resolvedSchemas was null");
        }
        Map<String, SoftReference<XmlSchema>> threadHT = SchemaBuilderCacheTest.getThreadResolvedSchemaHashtable();
        if (threadHT == null ) {
            testMonitor.t1Exception = new Exception("Thread resolvedSchemas was null");
        }
        
        if (threadHT.isEmpty()) {
            testMonitor.t1Exception = new Exception("Thread resolvedSchemas was empty");
        }
        ...
        // After the other thread does a reset, the cache for this thread should NOT be null
        if (SchemaBuilderCacheTest.getResolvedSchemasHashtable() == null) {
            testMonitor.t1Exception = new Exception("resolvedSchemas was null after reset");
        }
        threadHT = SchemaBuilderCacheTest.getThreadResolvedSchemaHashtable();
        if (threadHT == null ) {
            testMonitor.t1Exception = new Exception("Thread resolvedSchemas was null after clear");
        }
        
        if (threadHT.isEmpty()) {
            testMonitor.t1Exception = new Exception("Thread resolvedSchemas was empty after clear");
        }
        
        // Issue our a clear on this TID, and now there should be no entires for it
        SchemaBuilder.clearCache();
        if (SchemaBuilderCacheTest.getResolvedSchemasHashtable() == null) {
            testMonitor.t1Exception = new Exception("resolvedSchemas was null after clear on TID");
        }

        threadHT = SchemaBuilderCacheTest.getThreadResolvedSchemaHashtable();
        if (threadHT != null ) {
            testMonitor.t1Exception = new Exception("Thread resolvedSchemas was not null after clear");
        }
    }
}
 ...
 * - Initialize the cache then make sure it was used for a resolve on this thread
...
class TestingRunnable2 implements Runnable {
    ...
    public void run() {
        ...
        Map<String, SoftReference<XmlSchema>> threadHT = SchemaBuilderCacheTest.getThreadResolvedSchemaHashtable();
        if (threadHT == null ) {
            testMonitor.t2Exception = new Exception("Thread resolvedSchemas was null");
        }
        if (threadHT.isEmpty()) {
            testMonitor.t2Exception = new Exception("Thread resolvedSchemas was empty");
        }

        // Issue our a clear on this TID, and now there should be no entires for it
        SchemaBuilder.clearCache();
        if (SchemaBuilderCacheTest.getResolvedSchemasHashtable() == null) {
            testMonitor.t2Exception = new Exception("resolvedSchemas was null after clear on TID");
        }

        threadHT = SchemaBuilderCacheTest.getThreadResolvedSchemaHashtable();
        if (threadHT != null ) {
            testMonitor.t2Exception = new Exception("Thread resolvedSchemas was not null after clear");
        }
...
/**
 * Test that the init done by Thread 1 does NOT cause the cache to be used by this thread 
...
class TestingRunnable3 implements Runnable {
    ...
    public void run() {
        ...
        Map<String, SoftReference<XmlSchema>> threadHT = SchemaBuilderCacheTest.getThreadResolvedSchemaHashtable();
        if (threadHT != null ) {
            testMonitor.t3Exception = new Exception("Thread resolvedSchemas was not null");
        }
